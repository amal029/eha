...
def robot():
    ...
    pnx = (lambda x: x[0] + numpy.random.rand()*0.00 +
           (0 if(gn(x[0], x[1])) else cos(x[2])*x[4]*d))

    # FIXME: I might need to fix simulink -- make it discrete

    # XXX: The evolution of y
    py = (lambda x: x[1] + If(g(x[0], x[1]), 0, sin(x[2])*-x[5])*d)
    pny = (lambda x: x[1] + numpy.random.rand()*0.00 +
           (0 if(gn(x[0], x[1])) else sin(x[2])*-x[5]*d))
    # XXX: The evolution of th
    pth = (lambda x: x[2] + If(g(x[0], x[1]), 0, tan(x[3])/ll*x[6])*d)
    pnth = (lambda x: x[2] + numpy.random.rand()*0.00 +
            (0 if(gn(x[0], x[1])) else tan(x[3])/ll*x[6]*d))
    # XXX: Evolution of ph
    pph = (lambda x: x[3] + If(g(x[0], x[1]), 0, -x[7])*d)
    pnph = (lambda x: x[3] + numpy.random.rand()*0.00 +
            (0 if(gn(x[0], x[1])) else -x[7]*d))
    ...
    count = 1
    # XXX: The solver
    s = SMPC.MPC(N, 4, 4, ps, xl, xu, ul, uu, norm=1)
    print(x0)
    while(True):
        print('------------l∞ norm cost function-------------')
        u0 = s.solve(x0, rxs, rus, wx, wu)
